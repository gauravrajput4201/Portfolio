...
const Navbar = () => {
  ...
  <div className="max-w-[1400px] mx-auto flex justify-between items-center gap-8">
        ...
        >
          <span className="font-inter tracking-tight">AR</span>
        </motion.div>
        ...
        <div className="hidden md:flex gap-8 items-center">
          {navItems.map((item, index) => (
            <motion.button
              key={item.id}
              className="relative text-white/80 text-[0.95rem] font-medium bg-none border-none cursor-pointer py-2 transition-colors duration-300 hover:text-white"
            ...
            >
              {item.label}
              <span className="absolute bottom-0 left-0 w-0 h-[2px] bg-gradient-to-r from-[#06b6d4] to-[#8b5cf6] transition-all duration-300 group-hover:w-full"></span>
            </motion.button>
          ))}
        </div>
